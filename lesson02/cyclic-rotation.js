...
[6, 3, 8, 9, 7] 
(elements are shifted right by one index and 6 is moved
     to the first place).
...
export const rotate = (A, k) => {
  const n = A.length;
  const indexToCut = Math.abs(n - k) % n;
  if (indexToCut === 0 || n === 0) {
    return A;
  }

  const arr = [...A.slice(indexToCut, n), ...A.slice(0, indexToCut)];
  return arr;
};
